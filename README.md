...
[![tests](https://img.shields.io/badge/tests-293%20tests-3fb950)](../../actions)
[![release](https://img.shields.io/badge/release-v1.16.0-0066cc)](../../releases)
...
이 검토를 공통 디자인 시스템과 브라우저 렌더 검사까지 이어 갑니다.
...
PDF는 이 HTML을 자동화 브라우저로 인쇄해 생성합니다.

아래 이미지는 같은 가상 원고를 브라우저 기본 스타일로 저장한 HTML과 `paper` 모드로
`typesetting`한 결과를 나란히 보여 줍니다.
...
- 제공된 수치와 서술의 정합성은 검사하지만, 입력 자료에 없는 사실이나 근거를 새로
  확정하지 않습니다.
- 기준 결과물은 자립형 HTML이며, 필요하면 PDF를 함께 생성합니다. 네이티브 `.docx`와
  `.pptx` 파일은 생성하지 않습니다.
