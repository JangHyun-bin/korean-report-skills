...
내용은 올바르더라도 번역체, 모호한 수치, 들쭉날쭉한 페이지 배치가 남으면 문서는 아직
초안처럼 보입니다.

`korean-report-style`은 문체와 근거를 다듬고, `korean-report-doc`은 그 내용을
일관된 `layout`으로 구성한다. 이어서 HTML `typesetting`과 렌더 검사를 수행하고,
필요하면 PDF와 검수용 스크린샷까지 만듭니다.

여기서 `layout`은 페이지와 요소의 배치를, `typesetting`은 CSS·글꼴·수식을 반영해
최종 HTML을 만드는 처리 단계를 뜻한다.

글만 다듬을 때는 `korean-report-style`을 사용한다. HTML이나 PDF까지 필요할 때는
`korean-report-doc`을 함께 사용한다. Claude Code, Codex, Cursor, OpenCode에서
사용할 수 있다.
...
스킬 이름을 직접 적을 필요는 없다. 명시적으로 선택하려면 Codex에서는
`$korean-report-doc`, Claude Code에서는 `/korean-report-doc`처럼 요청할 수 있다.

설명이나 요약만 요청하면 대화 안에서 답하고, 별도의 문서 파일은 만들지 않는다.
HTML이나 PDF처럼 완성된 산출물을 요청하면 문서 제작 절차를 시작한다.

## 결과물

문서의 목적에 따라 `paper`와 `deck` 중 하나를 선택한다.
...
완성된 HTML은 CSS, 사용된 KaTeX CSS와 글꼴을 한 파일 안에 담는다. 실행할 때
외부 자원을 요청하지 않으므로 네트워크가 없는 환경에서도 열고 인쇄할 수 있다.
PDF는 이 HTML을 자동화 브라우저로 인쇄해 만듭니다.

아래 이미지는 같은 가상 원고를 브라우저 기본 스타일로 저장한 HTML과 `paper` 모드로
`typesetting`한 결과를 나란히 보여준다.
...
있다. 예시에 등장하는 기업, 공정, 인물, 수치는 모두 가상이다.
...
`opencode.json`의 `plugin` 배열에 npm 패키지를 등록한다.
...
OpenCode를 다시 시작하면 package의 `config` hook이 두 스킬의 경로를 추가한다.
이 방식에는 별도의 `npm i -g`나 `npx` 복사가 필요하지 않는다.

### Cursor와 파일 복사 설치

`npx` 설치기는 Claude Code, Codex, Cursor의 스킬 디렉터리에 두 스킬을 복사한다.
OpenCode의 npm 플러그인 등록과는 다른 설치 방식이다.
...
`korean-report-style`을 확인한다. 갱신, 제거, 프로젝트 설치, claude.ai 업로드 절차는
[INSTALL.md](INSTALL.md)에 정리되어 있다.
...
`korean-report-style`의 lint는 Markdown과 HTML의 가시 텍스트를 검사한다.
치환 목록 118건과 제목, 문맥 의존 표현, 어미 규칙을 네 갈래로 구분한다.
...
검사 결과는 일반 텍스트뿐 아니라 JSON, GitHub annotation, SARIF로도 출력할 수 있다.
의미가 달라질 수 있는 용어나 문맥 의존 표현은 자동으로 수정하지 않는다.

### 렌더 검사

HTML을 브라우저로 렌더한 뒤에는 다음 항목을 확인한다.
...
판정하기 어려운 항목은 스크린샷과 PDF를 함께 확인한다.

## 지원 범위와 현재 한계

문서 제작에는 진행현황, 중간보고, 기술보고, 연구노트, 벤치마크 보고, 협의자료,
회의 아젠다, 제안서가 포함된다. 문체 규칙은 코드베이스 인수인계, 아키텍처 문서,
runbook, API reference, 모델 평가, 회의록, 컨소시엄 산출물에도 적용할 수 있다.

현재 범위는 다음과 같다.

- 한국어 기술·사업 문서를 대상으로 한다. 다른 언어의 문체와 `layout` 규칙은 아직
  포함하지 않는다.
- 제공된 수치와 서술의 정합성은 검사하지만, 입력 자료에 없는 사실이나 근거를 새로
  확정하지 않는다.
- 기준 결과물은 자립형 HTML이며, 필요하면 PDF를 함께 만듭니다. 네이티브 `.docx`와
  `.pptx` 파일은 생성하지 않는다.
- 법률·규제 적합성이나 외부 사실의 정확성을 보증하지 않는다. 기한, 의무, 책임이
  포함된 문장은 사용자가 제공한 근거 문서와 대조해야 한다.

본문 글꼴은 사용 권한이 있는 WOFF2 파일을 `--font`로 지정한 경우에만 HTML에
내장된다. 글꼴을 지정하지 않아도 빌드는 완료되지만 시스템 글꼴을 사용하므로
기기마다 줄바꿈과 페이지 배치가 달라질 수 있다.

## 필요한 실행 환경

`korean-report-style`의 문체 규칙에는 추가 런타임 의존성이 없다.
`korean-report-doc`으로 HTML을 빌드하려면 Node 20 이상과 KaTeX가 필요한다.
렌더 QA, 스크린샷, PDF 출력에는 Python 3.11 이상, Playwright, Chromium이 필요한다.
...
저장소의 Node 의존성은 `npm install`로 설치한다. 위의 Python 실행 환경을 준비한 뒤
다음 명령으로 문서 뼈대를 만들 수 있다.
...
생성된 Python 파일에서 본문, 표, 도해를 수정한다. 본문 글꼴을 HTML에 내장하려면
실행할 때 WOFF2 경로를 지정한다.
...
문체 검사와 저장소 회귀 검사는 다음 명령으로 실행할 수 있다.
...
문체 규칙은 Markdown 표를 단일 원천으로 사용한다. `base.css`에는 공통 디자인 token,
`paper.css`와 `deck.css`에는 모드별 규칙이 있다. 도해의 SVG class도 같은 token을
사용한다.

새 문체 규칙과 문맥 heuristic은 현재 구조 안에서 추가할 수 있다. 새 출력 모드,
글꼴 정책, 언어 지원을 추가하려면 template, builder, QA, 회귀 검사를 함께 변경해야
한다.

## 기여와 보안

기여 절차와 회귀 검사 기준은 [CONTRIBUTING.md](CONTRIBUTING.md)에 있다. 보안
취약점은 공개 issue 대신 [Security Advisory](../../security/advisories/new)로 신고한다.
세부 정책은 [SECURITY.md](SECURITY.md)에서 확인할 수 있다.
