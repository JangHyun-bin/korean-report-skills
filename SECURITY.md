...
여기 담긴 것은 라이브러리가 아니라 **에이전트가 읽고 그대로 따르는 지시문 **입니다.
`SKILL.md` 한 줄을 바꾸면 그 스킬을 설치한 모든 사람의 에이전트 행동이 바뀝니다.
일반적인 코드 취약점보다 이쪽 경로가 더 중요합니다.
...
- **산출물 오염** — 생성된 HTML에 의도하지 않은 스크립트·외부 요청이 들어가는 경우.
  이 저장소는 **런타임 네트워크 의존 0**을 원칙으로 하며, 검사가 이를 강제합니다
- **공급망** — `.skill` 아카이브가 `skills/` 폴더와 다른 내용을 담는 경우.
  `bash scripts/pack-skills.sh --verify` 가 대조합니다

## 신고

취약점은 **공개 이슈로 올리지 않습니다.**
...
첫 응답까지 영업일 기준 5일을 목표로 합니다.

## 설치자가 확인할 것

스킬은 출처를 모르면 설치하지 않습니다. 이 저장소는 다음을 지킵니다.

- **설치 시 자동 실행되는 코드가 없습니다.** `scripts/install.sh` 는 파일 복사만 합니다
- 실행 자산은 `figures.py`(문자열 생성) · `mathbuild.js`(빌드) 뿐이며,
  둘 다 네트워크에 접근하지 않습니다
- 생성 문서는 자립형입니다 — 외부 자원을 런타임에 부르지 않습니다
...
최신 릴리스만 지원합니다. 이전 버전에는 수정을 소급하지 않습니다.
